...
在这一小节中，你要先记住两个小东西，先记住即可，不要管它是什么。
第一个是**LHS查询**
第二个是**RHS查询**

其中，**L**和**R**你可以理解为左和右。
那么...在哪里LHS查询...又在哪里RHS查询呢...

当当当当！**是在赋值符号(也就=号)的左侧和右侧。**


在我们了解过引擎和编译器的基本流程后，再让我们以一种新的方式思考，引擎是如何查找一个变量和给变量赋值的。
...
在上面的代码中，**引擎**会对变量`a`进行LHS查询。

没错！就是辣么简单！这就是**LHS查询**,当然你会有点懵，这是怎么肥事，怎么就**LHS查询**了,让我们用通俗的语言来讲就是，**当变量出现在赋值符号(操作)的左面时，引擎会对变量进行LHS查询，反之，进行RHS查询。**

###### 注意：这只是我方便你的理解才这么说的，在后面我们会对LHS和RHS有一个新的认识。
...
1. 引擎对`a`进行LHS查询
2. 引擎对`b`进行LHS查询
3. 引擎对`b`赋值操作的右侧中的`a`进行RHS引用

你对于第一二步应该没有什么疑问，那么第三步可能会有疑问，让我们来一起分析一下。

第二行代码中先对`b`进行`LHS引用`,而后,**当引擎看到赋值操作的右侧是一个变量，而非是一个值，那么就会对赋值操作右侧的`a`进行RHS引用。**


**那么...到这里，我们已经简单的认识到了什么是LHS，什么是RHS，接下来，我们在深入理解一下吧！**
...
console.log(a)
```
上面这行代码，引擎对`a`的引用到底是用LHS还是RHS呢？
答案是RHS!

你可能会说，可是这里并没有赋值操作啊！！！

那么，到这里，让我们重新认识一下RHS查询。

在我们上面的说明中，对变量进行LHS查询理解为**找到某个变量，从而进行赋值。**

而RHS查询则是简单的查找到某个变量的值，如果说的更严格一点，则是**RSH并非是赋值的右侧，而是"非左侧"**。

| 我们将RHS理解为retrieve his source value.取到源值，也就是说，取到某某的值。

---
结合上面的结论，让我们在回到第二行代码中,因为`b = a`中的`a`出现在非左侧，所以引擎会对`a`进行RHS引用，所以才会是RHS查询！
